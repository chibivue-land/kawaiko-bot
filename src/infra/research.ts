...
/**
 * The ResearchService port: a lookup that returns "" whenever the question does
 * not warrant one, so callers never have to ask twice.
 */
export function webResearchService(opts: { githubToken?: string }) {
  return {
    async lookup(question: string): Promise<string> {
      if (!question || !needsResearch(question)) return "";
      return gatherResearch(question, opts).catch(() => "");
    },
  };
}
