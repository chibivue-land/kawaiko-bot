...
- スラッシュコマンドではなく **アカウントとして** 振る舞う
  ...
  - 数時間に 1 回，住処チャンネルの誰かの発言に頼まれてもいないのに絡む (同じ発言に 2 度は絡まない)
  - `@kawaiko reset` (`リセット` / `忘れて` も可) で **そのチャンネルだけ** 会話を忘れる
  - `@kawaiko jev <命題>` で，判定モデル [Jev](#判定器-jev) に直に判定させる
- サーバーのことを覚えていく．会話を観測ログに貯め，毎時「持続的な事実」だけを追記専用ログへ畳み込む
- 同じ型・同じ分量に収束しないよう，生成のたびに返しの型と長さを振り直し，直前の自分の発言と被ったら作り直す
- 返事の前に発言を**見立てる**．雑談は無料枠の小さいモデルで即レスし，しりとりの一手や計算のように正解のある発言だけ，賢いモデルに思考つきで当てる
- 辛口だが，暗い話ばかりはしない．ダジャレ・ボケ・風刺・批評が主食で，流行も追う．ごく低い確率で乱心する
...
| AI         | `モデル一覧` の優先順で試す．Workers AI (無料枠) → Gemini．どこで失敗しても次へ渡す．正解のある発言だけ `熟考のモデル一覧` を先に試す |
| 判定       | [Jev](#判定器-jev) (TypeSafe の判定モデル)．AI Gateway 越しの Workers AI binding．見立て・検品・相手選び・門番・`jev` 命令            |
...
- 呟きの時刻と頻度: [wrangler.jsonc](wrangler.jsonc) の `triggers.crons` と `独言の確率` / `横槍の確率`
- 乱心の頻度: `乱心の確率` (既定 0.05)．雑談への返事・独言・横槍でだけ賽を振る．正解のある問いや，調べて答える問いの最中には乱心しない．文面は [源/核/機嫌-乱心.md](源/核/機嫌-乱心.md)
- 人格の調整: [源/核/人格-語彙集.md](源/核/人格-語彙集.md) と [源/核/人格-指示書.md](源/核/人格-指示書.md)．コードではなく文章を書き換える
  - 返しの型と長さのダイヤル: [源/核/人格-抑揚.json](源/核/人格-抑揚.json)．`答える型` は，中身のある答えが要る場面で使ってよいものだけの抜粋
  - 独り言のお題: [源/核/独言-お題.json](源/核/独言-お題.json)．書き出しが `ニュース:` / `技術の流行:` / `世間の流行:` のお題は，その種類の見出しを取ってきて添える (購読先は [源/外界/見出し.ts](源/外界/見出し.ts))．事故・訃報・人の醜聞の見出しは，語と判定器の二段で落としてから渡す
- メンション応答は bot が読めるどのチャンネルでも動く．呟きと横槍は `住処のチャンネルid` のみ
- 横槍は，kawaiko がもう返信した発言には二度と絡まない (履歴の返信先を見て候補から外す)
...
調べると決まったら、巷のコーディングエージェントの web 検索と同じ**探索ループ**に入る ([源/振る舞い/探索.ts](源/振る舞い/探索.ts))。引いて開くを一回ずつ機械的にやるのではなく、検索の結果を見せてから次の一手をモデルに選ばせる:
...
発言 ──▶ 道案内 (llama-3.2-3b) ──▶ 調べる? ──いいえ──▶ そのまま答える
                                     ...
                                     はい
                                      ▼
                       初手の検索語で引く (Bing を Browser Run で
                          + News / GitHub / HN の補助)
                                      │
                                      ▼
             ┌────────▶ 結果を見て次の一手を決める (モデル)
...
抜粋で終わらせず**実際にページを開く**のが肝です。[Browser Run](https://developers.cloudflare.com/browser-run/) は素の `fetch` と違って本物のブラウザで開くので、JS で組み立てるページも中身が取れます。開けるのは**検索結果に出てきた URL だけ** — モデルが作り話の URL を言っても開きに行きません ([源/核/探索.ts](源/核/探索.ts))。

判断にはもう 1 票、[判定器 (Jev)](#判定器-jev) が入ります。「要る」と言えば調べ、はっきり「要らない」と言ったときだけ**語の当てはめの票を取り消す** — «なにそれ草？» で探索ループを回さないため。道案内の票までは取り消しません (調べ損ねのほうが悪いので)。

道案内が使えないときは、これまでどおりの語の当てはめへ落ちます ([源/核/調べ方.ts](源/核/調べ方.ts))。判断できなかったことを理由に、調べ物ごと諦めはしません。ループの判断が読めないときも同じで、そこまでに集めた材料で答えます。

費用: 道案内は 4,625 neuron/M 入力トークンなので 1 回あたり 1 neuron に届きません。ブラウザの時間は Workers Free だと **1 日 10 分**なので、ループには天井がある — 1 回の返事につき検索 3 回・開くのは 2 ページ・判断 4 手まで ([源/核/探索.ts](源/核/探索.ts))。ループの判断に使ったモデル代も予算番に載ります。

### 判定器 (Jev)

[Jev](https://developers.cloudflare.com/ai/models/typesafe/jev/) は文章を書かないモデル．状態を型付きの問い — 真偽 (Noul)・選択 (Choice)・段階 (Score) — に照らして，確率つきの答えを 1 秒かからずに返す．入力 100 万トークンで $0.042，出力は無料．kawaiko では「書く係」(Workers AI / Gemini) の前後に挟む「決める係」として使う:

| 使いどころ     | 訊くこと                                         | 効き方                                                                                                          |
| -------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| 構えの見立て   | どれだけ注意深く考える必要があるか               | 構え (反射・普通・熟考) を決める．熟考なら賢いモデル + 思考つき + 口調のダイヤル無し                            |
| 調べるかの票   | 外へ裏を取りに行くべきか                         | [調べて答える](#調べて答える) の判断に 1 票．「要る」なら調べ，はっきり「要らない」なら当て推量の票だけ取り消す |
| 返事の検品     | 返事が相手の求めに応じているか                   | 応じていなければ，構えを熟考へ上げて 1 回だけ作り直す                                                           |
| 横槍の相手選び | 絡みがい / そっとしておくべきか                  | 落ち込んでいる人は避け，絡みがいで重みを付けて選ぶ                                                              |
| 学習の門番     | 持続するか / 指示が紛れているか / 踏み込みすぎか | その場のノリ・事実の顔をした指示・私的な情報を，覚える前に落とす                                                |
| 見出しの選別   | 冗談のネタにしてはいけない話題か                 | 事故・訃報・人の醜聞を，独り言のネタから外す                                                                    |
| `jev` 命令     | 人が書いた問いそのもの                           | 答えを生成を挟まずにそのまま返す                                                                                |

問いの文面と閾値は，それを使うモジュールの隣の JSON にある ([見立て](源/核/見立て-問い.json)・[検品](源/核/検品-問い.json)・[横槍](源/核/横槍-問い.json)・[門番](源/核/学習-門番.json)・[見出し](源/核/独言-見出しの問い.json))．調整はコードではなくそちらで．

**前提**: Jev は第三者モデルなので，AI Gateway の **Unified Billing の残高**から引かれる (ダッシュボードの AI > AI Gateway > Credits でチャージ)．`default` のゲートウェイは最初の呼び出しで勝手に作られる．残高が無いアカウントでは呼び出しが転ぶが，判定はどれも上乗せなので kawaiko は止まらない — 転んだ判定器は 30 分休ませ，そのあいだは決定的な合図 (遊びの名前・問いかけの語・不謹慎な語の一覧) で動く．休んでいる理由は認可つきの `GET /status` の「休んでいる提供者」に `jev` として出る．最初から置きたくなければ `判定のモデル` を `"none"` に．

チャットから直に叩く:

```
@kawaiko jev この発言は煽りか
@kawaiko jev 今日の晩飯 [ラーメン|カレー|寿司]
@kawaiko jev このコードのヤバさ [平和<怪しい<炎上]
```

角括弧なしは真偽，`|` 区切りは選択，`<` 区切りは段階 (低いほうから)．誰かの発言に返信で付ければその発言を，そうでなければ直近の流れを判定する．

端末から叩く (`TRIGGER_TOKEN` で認証．`q` は `jev` 命令の中身，`state` は判定に掛ける状態):

```bash
curl -s -H "Authorization: Bearer $TRIGGER_TOKEN" -d '{"q": "この文は煽りか", "state": "お前の実装，全部ダメ"}' "https://kawaiko-bot.<subdomain>.workers.dev/jev"
```

### 返信を辿る

メンションが**誰かの発言への返信**として来たら、`message_reference` を親まで遡って連なりを読み、指示文に古い順で載せる ([源/核/転記.ts](源/核/転記.ts))。どの発言への返事かを直近ログの推測に任せると、古い話への返信で取り違えるので。遡るのは 5 通まで。途中の発言が消えていたら、読めたところまでで打ち切ります。
...
| `POST /jev`            | 要   | 判定器 (Jev) を直に叩く．本体は `{"q", "state"}`        |
