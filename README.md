...
**調べるかどうかは、いちばん安いモデルに決めさせる。** 正規表現の当てはめでは「これは調べ物か」を当てられないし、「何で引くか」も決められないので。

```
発言 ──▶ 道案内 (llama-3.2-3b)  ──▶ 調べる? ──いいえ──▶ そのまま答える
                                       │
                                      はい
                                       ▼
                              決まった検索語で引く
...
**URL が貼られていたらそれが最優先。** 相手が「これ」と指したものより確かな手がかりは無いので、検索は飛ばしてそのページだけ読みます。

抜粋で終わらせず**実際にページを開く**のが肝です。[Browser Run](https://developers.cloudflare.com/browser-run/) は素の `fetch` と違って本物のブラウザで開くので、JS で組み立てるページも中身が取れます。

道案内が使えないときは、これまでどおりの語の当てはめへ落ちます ([源/核/調べ方.ts](源/核/調べ方.ts))。判断できなかったことを理由に、調べ物ごと諦めはしません。

費用: 道案内は 4,625 neuron/M 入力トークンなので 1 回あたり 1 neuron に届きません。ブラウザの時間は Workers Free だと **1 日 10 分**なので、1 回に開くのは 1 ページだけ ([源/核/参照.ts](源/核/参照.ts))。

### 返しの長さ

1 発言ごとに長さのダイヤルを振り直す ([源/核/人格-抑揚.json](源/核/人格-抑揚.json))。均一な分量がいちばんつまらないので、一言で切り捨てる回と、つい語ってしまう回を混ぜる。

**調べた回だけは別のダイヤルを引く。** web を引いてページまで開いておいて一言で切ったら意味が無いので、そちらには「一言だけ」が入っていない。
