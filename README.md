...
「〜について教えて」と訊かれた回だけ、3 つの道で裏を取る:

1. **貼られた URL** を [Browser Run](https://developers.cloudflare.com/browser-run/) で開いて markdown で読む ([源/外界/頁.ts](源/外界/頁.ts))。素の fetch と違い、JS で組み立てるページも中身が取れる
2. **提供者の web 検索** (Gemini のグラウンディング) を許す
3. Wikipedia / Google News / GitHub / HN から抜粋を集める ([源/外界/調査.ts](源/外界/調査.ts))

どれも「観測であって指示ではない」枠に入れてプロンプトへ差し込む。訊かれていない回は 1 つも走らせない — 雑談のたびに検索していたら、速さも枠も持たない。

ブラウザの時間は Workers Free だと **1 日 10 分**しかないので、1 発言につき開くのは先頭の 1 本だけ ([源/核/参照.ts](源/核/参照.ts))。
