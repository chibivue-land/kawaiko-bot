 ...
 */
/**
 * 永続体 (Durable Object) の返事を，本物の約束に均す．
 *
 * RPC が返すのは約束ではなく，知らないプロパティを「向こう側のメソッド呼び出し」
 * として解釈する代物だった．そのまま `.んで` と書くと «んで» という名前のメソッドを
 * 永続体に探しにいって落ちる — 実際に本番で
 * `The RPC receiver does not implement the method "んで"` を出した．
 *
 * ポートは約束を返すと約束しているので，境界であるこのファイルで必ず均す．
 * 永続体を呼ぶ行がここ以外に増えたときも，同じように通すこと．
 */
const 約束に均す = 約束にする;
