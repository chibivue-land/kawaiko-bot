...
import type { 文字列, 数値, 真偽, 約束, 配列, 記録, 省略可, 空 } from "../共通/型";
...
const 問いかけの型 = new RegExp(繋ぐ(問いかけの合図, "|"));
...
const 知ってるか型 = new RegExp(`^(.+?)${敬称}${話題の助詞}${訊き方}`);

export function 検索語を取り出す(問いかけ: 文字列): 文字列 {
  const 主語 = 知ってるか型.exec(問いかけ)?.[1];
  const 削った =
    主語 === undefined ? undefined : 前後の空白を落とす(置き換える(主語, /[はがのをも]\s*$/, ""));

  return もし(削った !== undefined && 長さ(削った!) >= 2, {
    であれば: () => 削った!,
    でなければ: () => 問いかけ,
  });
}
...
async function 時間を切って取る(
  ...
  return 試みる<省略可<Response>>({
    実行: async () => {
      ...
      return もし(応答.ok, { であれば: () => 応答, でなければ: () => undefined });
    },
    しくじったら: () => undefined,
  });
}
  ...
  return もし(否定(/^[a-zA-Z0-9-]{2,39}$/.test(検索語)), {
    ...
    でなければ: async () => {
      ...
      return 応答から拾う(応答, async (応答) => {
        ...
        return もし(ひと.login === undefined, {
          であれば: (): 配列<調べもの> => [],
          でなければ: () => {
            const 断片 = 絞る(
              [
                ひと.name && ひと.name !== ひと.login ? `name: ${ひと.name}` : "",
                ひと.bio ? `bio: ${ひと.bio}` : "",
...
async function HackerNewsを引く(検索語: 文字列): 約束<配列<調べもの>> {
  ...
  return 応答から拾う(応答, async (応答) => {
    ...
    return 写す(
      絞る(中身.hits ?? [], (件) => 否定(件.title === undefined)),
