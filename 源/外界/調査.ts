 ...
 * 二度判定しなくて済む．
 *
 * 調べるかどうかの見立てが先に付いていれば (`要るか`) それに従う．上の合図の語は
 * 粗い — «なにそれ草» でも外へ 4 本取りに行ってしまうし，取ってきた無関係な見出しは
 * 小さいモデルを脱線させる．見立てが無いときだけ，合図の語へ落ちる．
 */
export function webの調査係(設定: { GitHubのトークン?: 省略可<文字列> }): 調査係 {
  return {
    調べる(問いかけ: 文字列, 要るか?: 省略可<真偽>): 約束<文字列> {
      return もし<約束<文字列>>(空か(問いかけ) || 否定(要るか ?? 調べる価値があるか(問いかけ)), {
        であれば: () => すぐ返す(""),
        でなければ: () =>
