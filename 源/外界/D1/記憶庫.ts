...
class D1記憶庫 implements 記憶庫 {
  ...
  async 学習を追記する(引数: {
    ...
    const 結果: 取り込み結果 = { 学習: 0, 更新: 0, 却下: 0 };

    // 指示文に載せる一部ではなく、信じていること全部と突き合わせる。
    const 既知 = await this.生きている事実(引数.サーバーid, { 上限: 500 });
    const 現在 = 現在時刻().epochMilliseconds;
    const 行: (typeof 記憶の出来事.$inferInsert)[] = [];

    for (const 事実 of 引数.事実一覧) {
      const 取り込み方 = 取り込み方を決める(事実, 既知);

      if (取り込み方.種別 === "却下") {
        結果.却下++;
        continue;
      }
      if (取り込み方.種別 === "改訂") 結果.更新++;
      else 結果.学習++;
    ...
    try {
      await this.db.insert(記憶の出来事).values(行);
    } catch (躓き) {
 ...
 * `if (環境.DB)` を書かずに済み、用意する前でも Worker はそのまま動く。
 */
