...
class D1記憶庫 implements 記憶庫 {
  ...
  constructor(素のdb: DrizzleD1Database) {
    // Drizzle のクエリは thenable であって約束ではない．取り次ぎ越しにすると
    // 連鎖の途中でもそのまま `.んで` が書ける．
    this.db = 取り次ぐ(素のdb);
  }
  ...
  private 観測を書く(行: 読み取り専用配列<観測の入力>): 約束<無> {
    return 試みる({
  ...
  未学習の観測(サーバーid: 文字列, これ以降: 数値, 上限 = 60): 約束<配列<観測>> {
    return 試みる({
      実行: () =>
        this.db
          .select()
  ...
  学習を追記する(引数: {
    ...
    return this.生きている事実(引数.サーバーid, { 上限: 500 }).んで((既知) =>
      this.判定して書く(引数, 既知),
    );
  }
  ...
  学習の履歴(サーバーid: 文字列, 上限 = 20): 約束<配列<学習の回>> {
    return 試みる({
      実行: () =>
        this.db
          .run(
        ...
        SELECT e.batch                                              AS batch,
         ...
         LIMIT ${上限}`,
          ...
          .んで((行) =>
            写す(行.results as unknown as 生の回[], (一行): 学習の回 => ({
              識別子: 一行.batch,
              時刻: 一行.at,
              事実の数: 一行.facts,
  ...
  学習の回を取り消す(サーバーid: 文字列, 識別子: 文字列, 理由?: 省略可<文字列>): 約束<真偽> {
  ...
  時点まで巻き戻す(サーバーid: 文字列, 連番: 数値, 理由?: 省略可<文字列>): 約束<真偽> {
    return this.取り消しを追記する({
      サーバーid,
      識別子: `rollback:${連番}`,
      種別: "rollback",
  ...
  private 取り消しを追記する(行: Omit<typeof 記憶の出来事.$inferInsert, "時刻">): 約束<真偽> {
    return 試みる({
      実行: () =>
        this.db
          .insert(記憶の出来事)
          .values({ ...行, 時刻: 現在時刻().epochMilliseconds })
          .んで(() => 真),
      しくじったら: (躓き) => {
