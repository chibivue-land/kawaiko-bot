...
export function D1の記憶庫(db: D1Database): 記憶庫 {
  return D1記憶庫を作る(drizzle(db));
}

/** class の `new` はここ 1 箇所だけに閉じ込める． */
function D1記憶庫を作る(orm: DrizzleD1Database): 記憶庫 {
  return new D1記憶庫(orm);
}
...
class D1記憶庫 implements 記憶庫 {
  ...
  async 学習の読み取り位置(サーバーid: 文字列): 約束<数値> {
    return 試みる({
      実行: async () => {
        const [行] = await this.db
          .select({ 位置: sql<数値 | 空>`MAX(${記憶の出来事.読んだ位置})` })
          .from(記憶の出来事)
    ...
    return 試みる({
      実行: async () => {
        ...
        return 写す(行, (一行): 観測 => ({
          ...
          発言者id: 一行.発言者id,
          発言者名: 一行.発言者名,
          kawaikoの発言か: 等しい(一行.kawaikoの発言か, 1),
          本文: 一行.本文,
          時刻: 一行.時刻,
        }));
      },
  ...
  async 学習を追記する(引数: {
    ...
    const 結果: 取り込み結果 = {
      学習: 長さ(絞る(判定済み, (件) => 等しい(件.取り込み方.種別, "学習"))),
      更新: 長さ(絞る(判定済み, (件) => 等しい(件.取り込み方.種別, "改訂"))),
      却下: 長さ(絞る(判定済み, (件) => 等しい(件.取り込み方.種別, "却下"))),
    };

    const 学ぶ行 = 写す(
      絞る(判定済み, (件) => 等しくない(件.取り込み方.種別, "却下")),
      ({ 事実, 取り込み方 }): typeof 記憶の出来事.$inferInsert => ({
        ...
        // ここは素の === でないと 置き換える連番 が見えない．
        置き換える連番: 取り込み方.種別 === "改訂" ? 取り込み方.置き換える連番 : 空,
        出典チャンネルid: 事実.出典チャンネルid ?? 空,
        出典発言id: 事実.出典発言id ?? 空,
        モデル: 引数.モデル ?? 空,
      }),
    );
