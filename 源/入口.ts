...
import { 場所, 応答, 数値, 文字列, 新しい場所, 新しい応答, 未定義, 真, 真偽, 要求 } from "./共通/型";
...
async function 調査を覗く(環境: 環境, 行き先: 場所): 約束<応答> {
  const 問いかけ = 行き先.searchParams.get("q") ?? "";

  return もし(空か(問いかけ), {
    であれば: async () => 新しい応答("q が無い", { status: 400 }),
    でなければ: async () => {
      const 中身 = await 部品を組み立てる(環境).調査係.調べる(問いかけ);

      return 新しい応答(中身 || "(何も出なかった)", {
...
async function 記憶の口(環境: 環境, 要求: 要求, 行き先: 場所): 約束<応答> {
    ...
    [
      {
        ...
        ならば: async () => 新しい応答("権限が無い", { status: 401 }),
      },
      {
        条件: () => 等しい(問い, 未定義),
...
async function 引き金を引く(
  ...
  return 振り分ける<応答>(
    [
      ...
      {
        条件: () => 等しい(仕事, 未定義),
        ならば: async () => 新しい応答("知らない引き金", { status: 400 }),
      },
      {
        // 同期の形．調査用に結果をそのまま応答へ出す．
        条件: () => 等しい(行き先.searchParams.get("wait"), "1"),
        ならば: async () => {
          const 結果 = await 仕事!(部品を組み立てる(環境), { 強制するか: 真 });
