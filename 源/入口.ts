...
function 調査を覗く(環境: 環境, 行き先: 場所): 約束<応答> {
  ...
  return もし<約束<応答>>(空か(問いかけ), {
    であれば: () => 約束にする(新しい応答("q が無い", { status: 400 })),
    でなければ: () =>
      部品を組み立てる(環境)
        .調査係.調べる(問いかけ)
        .んで((中身) =>
...
function 状態(環境: 環境): 約束<不明> {
  ...
  return 揃える([
    約束にする(予算の記録.予算を確認する(上限ドル)),
    約束にする(接続.状態()),
    約束にする(予算の記録.直近の結果()),
    約束にする(予算の記録.休んでいる提供者()),
...
function 接続を確かめる(環境: 環境): 約束<無> {
  // 永続体の RPC は本物の約束ではない．約束にする を通してから続きを書く
  // (詳しくは 外界/組み立て.ts の 約束に均す)．
  return 約束にする(環境.Discord接続.get(環境.Discord接続.idFromName("global")).確かめる()).んで(
    () => 未定義,
  );
}
