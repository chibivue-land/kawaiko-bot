...
/**
 * Promise.resolve — もう出来ている約束にする．
 *
 * 値をそのまま渡せば「即座に出来ている約束」になり，Drizzle のクエリビルダの
 * ような約束もどき (then は持つが Promise ではない) を渡せば普通の約束になる．
 * どちらも「これは約束である」と言い直しているだけなので，名前は 1 つでよい．
 */
export function 約束にする<値>(値候補: 値 | 約束もどき<値>): 約束<値> {
...
/**
 * Promise.reject — 初めから転んでいる約束．
 *
 * 式の中で「ここは失敗した」と言うためのもの．投げるほうの しくじる と対になる．
 */
export function しくじった約束<値 = 無>(躓き: 不明): 約束<値> {
